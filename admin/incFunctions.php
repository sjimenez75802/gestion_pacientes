	...
	*/
	...
	if(!function_exists('getTableList')){
		function getTableList($skip_authentication = false){
			$arrTables = array(   
				'patients' => 'Pacientes',
				'disease_symptoms' => 'Recetario médico',
	...
	function setupMembership(){
		...
		if(!$anon_group_id){
			sql("insert into membership_groups set name='" . makeSafe($anon_group) . "', allowSignup=0, needsApproval=0, description='Grupo anónimo creado automáticamente el " . @date("Y-m-d") . "'", $eo);
		...
		if(!$admin_group_id){
			sql("insert into membership_groups set name='" . makeSafe($admin_group) . "', allowSignup=0, needsApproval=1, description='Grupo de adminitradores creado el {$today}'", $eo);
